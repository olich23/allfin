...
async def load_references():
    ...
    expense_cats, income_cats = await load_categories()
    print("DEBUG: Projects loaded:", projects)
    print("DEBUG: Expense categories loaded:", expense_cats)
    print("DEBUG: Income categories loaded:", income_cats)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Найдём проект
    project_id = await find_project_id(
        context.bot_data['projects_list'],
        parsed_data["project_name"]
    )
    if not project_id:
        suggestions = suggest_similar_projects(
            context.bot_data['projects_list'],
            parsed_data["project_name"]
        )
        if suggestions:
            await update.message.reply_text(
                f"Проект '{parsed_data['project_name']}' не найден. "
                f"Возможно, вы имели в виду: {', '.join(suggestions)}"
            )
        else:
            await update.message.reply_text(
                f"Проект '{parsed_data['project_name']}' не найден в системе."
            )
        return

    # Предположим, что это расход — ищем в списке статей расходов
    category_id = await find_category_id(
        context.bot_data['expense_categories'],
        parsed_data["category_name"]
    )
    if not category_id:
        ...
        if suggestions:
            await update.message.reply_text(
                f"Статья расходов '{parsed_data['category_name']}' не найдена. "
                f"Возможно, вы имели в виду: {', '.join(suggestions)}"
            )
        else:
            await update.message.reply_text(
                f"Статья расходов '{parsed_data['category_name']}' не найдена в системе."
            )
        return
    ...
    plan_paid_date = parsed_data.get(
        "plan_paid_date",
...
def suggest_similar_projects(projects_list, query, max_suggestions=5):
    ...
    matches = process.extract(query, project_names,
                              limit=max_suggestions, score_cutoff=50)
...
def suggest_similar_categories(categories_list, query, max_suggestions=5):
    ...
    matches = process.extract(query, category_names,
                              limit=max_suggestions, score_cutoff=50)
    return [match[0] for match in matches]
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        projects_list, expense_categories, income_categories = await load_references()
